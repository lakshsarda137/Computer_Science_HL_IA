...
def create_output_pdf(pdf_path, coordinates, output_pdf_path):
    ...
    for page_num in range(coordinates['page_start'], coordinates['page_end'] + 1):
        ...
        if word_count < 10 or question_number_count > 1:  # Validation to discard images with less than 10 words or containing more than one question
            os.remove(img_path)
            continue
...
def main():
    pdf_directory = "past_papers"  # Directory containing past papers
    unit = "1.1"  # Example user input
    marks_required = 4  # Example user input
    ...
    for pdf_file in pdf_files:
        ...
        questions.extend(extract_questions_from_pdf(pdf_path))
    ...
    while question_marks < marks_required and question_marks < len(questions):
        for question_index, question in enumerate(questions):
            question_number = int(re.findall(r'^\d+', question)[0])
            if question_number in [1, 2, 3, 4] or question_number < 1 or question_number > 40 or question_index in processed_questions:
                continue
            max_score = 0
            best_match = None
            ...
            if max_score > 0.2:  # Example threshold
                pdf_path = os.path.join(pdf_directory, pdf_files[question_index // len(questions)])
