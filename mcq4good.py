...
# Step 1: Extract questions from the past paper PDF
...
# Step 2: Calculate similarity scores between syllabus details and questions
def calculate_similarity(syllabus_details, questions):
    vectorizer = TfidfVectorizer().fit_transform([item for sublist in syllabus_details for item in sublist] + questions)
    vectors = vectorizer.toarray()
    
    syllabus_vectors = vectors[:len(syllabus_details)]
    question_vectors = vectors[len(syllabus_details):]
    
    similarity_scores = cosine_similarity(syllabus_vectors, question_vectors)
    return similarity_scores

# Step 3: Locate and take screenshots of relevant questions
def locate_question(pdf_path, question_number):
    doc = fitz.open(pdf_path)
    ...
    if not question_start:
        print(f"Question {question_number} not found in the document.")
        return None
...
def create_output_pdf(pdf_path, coordinates, output_pdf_path):
    ...
    for page_num in range(coordinates['page_start'], coordinates['page_end'] + 1):
        ...
        if rect.width <= 0 or rect.height <= 0:
            print(f"Invalid rectangle dimensions on page {page_num}. Skipping this page.")
            continue
...
def main():
    ...
    unit = "1.1"  # Example user input
    marks_required = 5  # Example user input

    # Step 3: Syllabus details array
    syllabus_details = [
    ["1 States of matter",
    ...
    # Flatten syllabus details for selected unit
    selected_unit_details = []
    for detail in syllabus_details:
        if detail[0].startswith(unit.split('.')[0]):
            for sub_detail in detail[1:]:
                if sub_detail[0].startswith(unit):
                    selected_unit_details.extend(sub_detail[1])

    questions = extract_questions_from_pdf(pdf_path)
    similarity_scores = calculate_similarity(selected_unit_details, questions)

    question_marks = 0
    question_index = 0
    output_pdf_paths = []

    while question_marks < marks_required and question_index < len(questions):
        for i, score in enumerate(similarity_scores):
            if score[question_index] > 0.5:  # Example threshold
                question_number = question_index + 1
                coordinates = locate_question(pdf_path, question_number)
                if coordinates:
                    output_pdf_path = f"output_{question_number}.pdf"
                    create_output_pdf(pdf_path, coordinates, output_pdf_path)
                    output_pdf_paths.append(output_pdf_path)
                    question_marks += 1
                if question_marks >= marks_required:
                    break
        question_index += 1

    # Combine individual PDFs into a single PDF
    combined_output_pdf_path = "combined_output.pdf"
    c = canvas.Canvas(combined_output_pdf_path, pagesize=letter)
    for path in output_pdf_paths:
        c.drawImage(path, 0, 0, width=letter[0], height=letter[1])
        c.showPage()
    c.save()
